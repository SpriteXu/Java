...
<body>
	...
	<!-- 中间内容 -->
	<div class="container-fluid bigHead" id="maincart" style="position:relative;top:80px">
		...
		<%--
			...
			<form id="cartForm" action="cart/preOrder" method="post">
				...
				<div class="row">
					<div class="col-lg-11 col-md-4 col-sm-4"></div>
					<button type="button" class="btn btn-success" onclick="confirmPre();">去结算</button>
	...
	$(function(){
		$.ajax({
			...
			success : function(arr) {
				if(arr!=null&&arr!=""){
						var str='<tr>'+
						'<th>全选<input onclick="selectAll();" type="checkbox" id="allSelect"></th>'+
						'<th>商品图</th>'+
						'<th>商品名称</th>'+
						'<th>商品单价</th>'+
						'<th>商品数量</th>'+
					...
					for(var i=0;i<arr.length;i++){
						...
						'</div>'+
						'</td>'+
						'<td><img style="height:100px;" src="upload/'+arr[i].cartGoods.goodsImg+'" />'+
						...
						'<td><button type="button" onclick="reduceNum('+arr[i].cartId+')" class="btn btn-warning" id="reduce'+arr[i].cartId+'">-</button><span style="width:50px;height:25px;display:inline-block;" id="num'+arr[i].cartId+'">'+arr[i].cartNum+'</span><button type="button" onclick="addNum('+arr[i].cartId+');" class="btn btn-warning" id="add'+arr[i].cartId+'">+</button></td>'+
						'<td><span id="totalPrice'+arr[i].cartId+'">'+arr[i].cartNum*arr[i].cartGoods.goodsPrice+'</span>元</td>'+
						'<td><button class="btn btn-danger" type="button" onclick="deleteCart('+arr[i].cartId+')">删除</button></td>'+
						'</tr>';
					}
					...
					"<img src='upload/emptycart.png'><p><h3>购物车好空啊，赶紧去<a href='view/index'>挑选心爱的宝贝</a>吧!</h3></p></div></div>";
					$("#maincart").html(empty);
				}
			},
